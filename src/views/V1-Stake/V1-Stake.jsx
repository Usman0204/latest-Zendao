...
function V1Stake({ oldAssetsDetected, setMigrationModalOpen, hasActiveV1Bonds }) {
  ...
  const onChangeStake = async action => {
    ...
    if (action === "stake" && gweiValue.gt(ethers.utils.parseUnits(ohmBalance, "gwei"))) {
      return dispatch(error("You cannot stake more than your ZEN balance."));
    }

    if (action === "unstake" && gweiValue.gt(ethers.utils.parseUnits(sohmBalance, "gwei"))) {
      return dispatch(error("You cannot unstake more than your sZEN balance."));
    }
            ...
            <Grid item>
              <MetricCollection>
                ...
                <Metric
                  ...
                  metric={`${formattedCurrentIndex} ZEN`}
              ...
              {!address ? (
                <div className="stake-wallet-notification">
                  ...
                  <Typography variant="h6">
                    <Trans>Connect your wallet to stake ZEN</Trans>
                  </Typography>
                </div>
              ) : (
                <>
                    ...
                    <Box mt={"10px"}>
                      <Typography variant="body1" className="stake-note" color="textSecondary">
                        {view === 0 ? (
                          <>
                            {hasActiveV1Bonds
                              ? t`Once your current bonds have been claimed, you can migrate your assets to stake more ZEN`
                              : !oldAssetsDetected
                              ? t`All your assets are migrated`
                              : t`You must complete the migration of your assets to stake additional ZEN`}
                          </>
                        ) : (
                          <br />
                        )}
                    ...
                    <Box className="stake-action-row v1-row " display="flex" alignItems="center">
                      {address && !isAllowanceDataLoading ? (
                        !hasAllowance("sohm") && view === 1 ? (
                          <Box mt={"10px"}>
                            <Typography variant="body1" className="stake-note" color="textSecondary">
                              <>
                                <Trans>First time unstaking</Trans> <b>sZEN</b>?
                                <br />
                                <Trans>Please approve Olympus Dao to use your</Trans> <b>sZEN </b>
                      ...
                      <TabPanel value={view} index={1}>
                        {isAllowanceDataLoading ? (
                          ...
                          >
                            {txnButtonText(pendingTransactions, "unstaking", t`Unstake ZEN`)}
                          </Button>
                        ) : (
                          <Button
                            className="stake-button"
                  ...
                  <div className="stake-user-data">
                    <DataRow
                      ...
                      id="user-balance"
                      balance={`${trim(Number(ohmBalance), 4)} ZEN`}
                    ...
                    <Accordion className="stake-accordion" square>
                      <AccordionSummary expandIcon={<ExpandMore className="stake-expand" />}>
                        <DataRow
                          ...
                          balance={`${trimmedBalance} sZEN`}
                          isLoading={isAppLoading}
                        />
                      </AccordionSummary>
                      <AccordionDetails>
                        <DataRow
                          title={`${t`sZEN Balance`} (v1)`}
                          balance={`${trim(Number(sohmBalance), 4)} sZEN`}
                          indented
                          isLoading={isAppLoading}
                        />
                        {Number(fsohmBalance) > 0.00009 && (
                          <DataRow
                            title={`${t`gZEN Balance in Fuse`}`}
                            balance={`${trim(Number(fsohmBalance), 4)} gZEN`}
                            indented
                            isLoading={isAppLoading}
                          />
                        )}
                        {Number(wsohmBalance) > 0.0 && (
                          <DataRow
                            title={`${t`wsZEN Balance`} (v1)`}
                            balance={`${trim(Number(wsohmBalance), 4)} wsZEN`}
                            isLoading={isAppLoading}
                            indented
                          />
                        )}
                        {Number(fiatDaowsohmBalance) > 0.00009 && (
                          <DataRow
                            title={`${t`wsZEN Balance in FiatDAO`} (v1)`}
                            balance={`${trim(Number(fiatDaowsohmBalance), 4)} wsZEN`}
                            isLoading={isAppLoading}
                            indented
                          />
                        )}
                        <DataRow
                          title={`${t`sZEN Balance`} (v2)`}
                          balance={`${trim(Number(sohmV2Balance), 4)} sZEN`}
                          indented
                          isLoading={isAppLoading}
                        />
                        <DataRow
                          title={`${t`gZEN Balance`} (v2)`}
                          balance={`${trim(Number(gOhmBalance), 4)} gZEN`}
                          indented
                          isLoading={isAppLoading}
                        />
                      </AccordionDetails>
